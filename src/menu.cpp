...
namespace Menu {
    ...
    void DrawPerkSelector(bool* a_open)
    {
        ...
        for (auto& [perk, initialised] : Config::Forms::perk_init_map) {
            if (!perk) 
                continue;
...
void Menu::Settings::RenderOptions()
{
    ...
    SettingCheckbox(Label::show_spell_learn_notif.c_str(), Var::show_spell_learn_notif, set::show_spell_learn_notif, Tool::show_spell_learn_notif.c_str());
    ImGuiMCP::SameLine();
    SettingCheckbox(Label::only_in_combat.c_str(), Var::only_in_combat, set::only_in_combat, Tool::only_in_combat.c_str());

    SettingSlider(Label::spell_xp_multiplier.c_str(), Var::spell_xp_multiplier, 0.1f, 5.0f, "%.2f", set::spell_xp_multiplier, Tool::spell_xp_multiplier.c_str());
    SettingSlider(Label::concentration_xp_modifier.c_str(), Var::concentration_xp_modifier, 0.1f, 5.0f, "%.2f",set::concentration_xp_modifier, Tool::concentration_xp_modifier.c_str());
