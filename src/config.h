...
struct Settings : REX::TSingleton<Settings>
{
    ...
    static inline REX::TOML::Str spell_learn_perk{SPELL_LEARNING, "sSpellLearnPerk",
                                                  std::string("Skyrim.esm|0x108a44")};
    static inline REX::TOML::Bool log_to_console{SPELL_LEARNING, "bLogToConsole", false};

    void UpdateSettings(bool a_save)
    {
        const auto toml = REX::TSingleton<REX::FTomlSettingStore>::GetSingleton();
...
struct SpellData : REX::TSingleton<SpellData>
{
...
struct Forms : REX::TSingleton<Forms>
{

    static inline RE::BGSPerk* spell_learn_perk{nullptr};
    static inline std::unordered_set<RE::BGSPerk*> perk_init_map;
    ...
    void AddToSpellSet() const
    {
        if (IsSpergPatchActive)
        {
            ...
            {
                Forms::SPELL_LIST_SPERG->ForEachForm(
                ...
                REX::INFO("Registered {} from SPERG patch",
                          static_cast<int>(SpellData::spell_allow_set.size()) - static_cast<int>(size_default));
            }
        }
    }
    ...
    void LoadForms()
    {
        ...
        perk_init_map = StyyxUtil::PerkUtil::GetAllPlayablePerks();

        REX::INFO("Loaded {} playable perks", perk_init_map.size());
        ...
        {
            Settings::require_perk.SetValue(false);
            REX::WARN("Spell Learn Perk can not be found, check your setting in the .toml file. It needs to be "
                      "\"Modname.esp|FormID\" or EditorID of the perk");
            return;
        }
        ...
        {
            Settings::require_perk.SetValue(false);
            REX::WARN("Spell Learn Perk is not a perk, check your setting in the .toml file. It needs to be "
                      "\"Modname.esp|FormID\" or EditorID of the perk");
            return;
        }
